...
cd build-ci/binarybit-$BUILD_TARGET
...
if [ "$DIRECT_WINE_EXEC_TESTS" = "true" ]; then
  ...
  wine ./src/test/test_binarybit.exe
else
  if [ "$RUN_UNIT_TESTS_SEQUENTIAL" = "true" ]; then
    ./src/test/test_binarybit --catch_system_errors=no -l test_suite
  else
