...
impl PrivateKey {
    ...
    pub fn generate_binarybit(rng: &mut StdRng) -> Result<Self, BlsError> {
    ...
    #[cfg(feature = "binarybit_helpers")]
    pub fn generate_binarybit_many(count: usize, rng: &mut StdRng) -> Result<Vec<Self>, BlsError> {
        (0..count)
            .into_iter()
            .map(|_| Self::generate_binarybit(rng))
            .collect()
    }
