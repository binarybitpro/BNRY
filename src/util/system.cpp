...
const char * const BITCOIN_CONF_FILENAME = "binarybit.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //binarybit-tx using stdin
...
{
    // Windows: C:\Users\Username\AppData\Roaming\BinaryBit
    // macOS: ~/Library/Application Support/BinaryBit
    // Unix-like: ~/.binarybit
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BinaryBit";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/BinaryBit";
#else
    // Unix-like
    return pathRet / ".binarybit";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty binarybit.conf if it does not exist
