BinaryBit Core version v18.1.0
=========================
...
Support for the BIP70 Payment Protocol has been dropped from BinaryBit Qt.
...
The systemd init file (`contrib/init/binarybitd.service`) has been changed to use
`/var/lib/binarybitd` as the data directory instead of `~binarybit/.binarybit`. This
change makes BinaryBit Core more consistent with other services, and makes the
...
`datadir` option in `/etc/binarybit/binarybit.conf` will have no effect. This is
because the command line arguments specified in the init files take precedence
over the options specified in `/etc/binarybit/binarybit.conf`.
...
See the [release notes from v18.0.1](https://github.com/binarybitpro/BNRY/blob/v18.0.1/doc/release-notes.md) for a full description of the changes from the
...
- The `testnet` field in `binarybit-cli -getinfo` has been renamed to `chain` and now returns the current network name as defined in BIP70 (main, test, regtest).
...
See detailed [set of changes](https://github.com/binarybitpro/BNRY/compare/v18.0.2...binarybitpay:v18.1.0).
