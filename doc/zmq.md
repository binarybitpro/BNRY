...
The BinaryBit Core daemon can be configured to act as a trusted "border
router", implementing the binarybit wire protocol and relay, making
...
The ZeroMQ feature in BinaryBit Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building binarybitd:
...
For instance:
    ...
    $ binarybitd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubrawtx=ipc:///tmp/binarybitd.tx.raw \
...
These options can also be provided in binarybit.conf.
...
From the perspective of binarybitd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into binarybitd directly. Furthermore, no information is
...
using. BinaryBitd appends an up-counting sequence number to each
